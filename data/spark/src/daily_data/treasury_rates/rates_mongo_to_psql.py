...
df = spark.read.format('mongo').option('spark.mongodb.input.uri',f'mongodb://{MONGO_ADDRESS}/stonks.treasuryRatesToday').load()
...
df.write.format('avro').save(os.path.join(f's3a://{AWS_BUCKET_NAME}/treasury_rates.avro'),mode="append")
