...
POSTGRES_HOST=os.getenv('AWS_PSQL_ADDRESS_1')
...
df = spark.read.format('mongo').option('spark.mongodb.input.uri',f'mongodb://{MONGO_ADDRESS}/stonks.sentimentToday').load()
...
.option("dbtable", "staging_sentiment_indicators") \
...
df.write.format('avro').save(os.path.join(f's3a://{AWS_BUCKET_NAME}/sentiment_data.avro'),mode="append")
