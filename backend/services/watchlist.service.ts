...
export class WatchlistService {
  ...
  async createWatchlist(userId: number, name: string): Promise<number> {
    return (await this.knex("watchlists").insert({ user_id: userId, name }).returning(["id"]))[0].id;
  }

  async changeWatchlistName(watchlistId: number, name: string, userId: number): Promise<void> {
    await this.knex("watchlist").update(name).where({ id: watchlistId }).andWhere({ user_id: userId });
  }
  ...
  async deleteWatchlist(watchlistId: number, userId: number): Promise<void> {
    ...
    await this.knex("watchlists").delete().where("id", watchlistId).andWhere({ user_id: userId });
  }
  ...
  async addStock(watchlistId: number, stockId: number): Promise<void> {
    await this.knex("watchlist_stock").insert({ watchlist_id: watchlistId, stock_id: stockId });
  }

  async deleteStock(watchlistId: number, stockId: number): Promise<void> {
